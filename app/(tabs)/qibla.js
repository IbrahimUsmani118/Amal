...
import { qiblaApi } from '@/services/qiblaApi';
...
export default function QiblaScreen() {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            realTimeDataService.removeListener('qibla', handleQiblaUpdate);
            realTimeDataService.removeListener('location', handleLocationUpdate);
            realTimeDataService.removeListener('qiblaError', handleQiblaError);
        };
    }, []);

    const initializeRealTimeData = () => {
        // Add listeners for real-time updates
        realTimeDataService.addListener('qibla', handleQiblaUpdate);
        realTimeDataService.addListener('location', handleLocationUpdate);
        realTimeDataService.addListener('qiblaError', handleQiblaError);

        // Get initial data
        const currentLocation = realTimeDataService.getCurrentLocation();
        const currentQibla = realTimeDataService.getQiblaDirection();
        ...
        if (currentQibla) {
            setQiblaDirection(currentQibla);
        }

        setRealTimeStatus('active');
    };

    const handleQiblaUpdate = (data) => {
        console.log('🧭 Real-time qibla update:', data);
    ...
    const handleQiblaError = (data) => {
        console.error('❌ Qibla error:', data);
        Alert.alert('Error', `Failed to update qibla direction: ${data.error}`);
    };
    ...
    const getUserLocation = async () => {
        try {
            ...
            console.log('Could not get user location, using Mecca:', error);
            setHasLocationPermission(false);
            // Fallback to Mecca coordinates
        ...
        try {
            ...
            if (location.city && location.country) {
                ...
                if (coordinates) {
                    ...
                    const partialMatches = Object.keys(cityCoordinates).filter(
                        (city) =>
    ...
    const getCityNameFromCoordinates = (lat, lng) => {
        ...
        Object.entries(cityCoordinates).forEach(([cityName, coords]) => {
            ...
            if (distance < minDistance && distance < 0.5) {
                // Within ~55km for better matching
                minDistance = distance;
                closestCity = cityName;
            }
        });

        // If no close city found, return coordinates
            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.statusContainer}>
                        <View style={[styles.statusDot, { backgroundColor: hasLocationPermission ? colors.accent : colors.primary }]} />
                        <Text style={[styles.statusText, { color: colors.textSecondary }]}>
                ...
                {qiblaDirection && (
                    <View style={[styles.infoCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                        <Text style={[styles.infoTitle, { color: colors.text }]}>About Qibla</Text>
                        <Text style={[styles.infoText, { color: colors.textSecondary }]}>
                            The Qibla is the direction that Muslims face when performing salah (prayer). This direction points towards the Kaaba in Mecca, Saudi Arabia.
                        </Text>
                        <View style={styles.infoRow}>
                            <Text style={[styles.infoLabel, { color: colors.text }]}>Your Location:</Text>
                            <Text style={[styles.infoValue, { color: colors.textSecondary }]}>
